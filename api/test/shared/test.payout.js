...
describe('Payout', () => {
    ...
    describe('Payout.calculateOwed', () => {
        ...
        it('calculates who owes correctly', () => {
            expect(_.find(amounts, { name: 'Adriana'}).owes).to.equal(18.85);
        });

       it('calculates who is owed correctly', () => {
           expect(_.find(amounts, { name: 'Camden'}).owed).to.equal(41.02);
       });
    });
    ...
    describe('Payout.calculatePayouts', () => {
        ...
        it('returns an array of Payout instances', () => {
            expect(payouts[0] instanceof Payout).to.equal(true);
        });

        it('calculates payouts correctly', () => {
           expect(payouts[0].amount).to.equal(18.85);
           expect(payouts[1].amount).to.equal(22.16);
        });
    });
});